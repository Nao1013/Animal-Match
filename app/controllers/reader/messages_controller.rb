class Reader::MessagesController < ApplicationController
  ...
  def show
    ...
    rooms = current_reader.rooms.pluck(:room_id) #ログイン中のユーザーの部屋情報を全て取得
    rooms = Room.find_by(facility_id: @facility.id, room_id: rooms)#その中にチャットする相手とのルームがあるか確認
    ...
    unless rooms.nil?#ユーザールームがある場合
      ...
      @room.save#そして保存
      Room.create(reader_id: current_reader.id)#自分の中間テーブルを作成
      Room.create(facility_id: @facility.id)#相手の中間テーブルを作成
    end
