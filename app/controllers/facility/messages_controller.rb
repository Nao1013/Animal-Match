class Facility::MessagesController < ApplicationController
  ...
  def show
   ...
   reader_rooms = ReaderRoom.find_by(reader_id: @reader.id, room_id: rooms)#その中にチャットする相手とのルームがあるか確認
   
   unless reader_rooms.nil?#ユーザールームがある場合
      @room = reader_rooms.room#変数@roomにユーザー（自分と相手）と紐づいているroomを代入
   else#ユーザールームが無かった場合
      @room = Room.new#新しくRoomを作る
      @room.save#そして保存
      Room.create(facility_id: current_facility.id, room_id: @room.id)#自分の中間テーブルを作成
      Room.create(reader_id: @reader.id, room_id: @room.id)#相手の中間テーブルを作成
   end
